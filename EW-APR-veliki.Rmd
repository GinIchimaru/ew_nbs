...
```
*Information value* od 0.94 deluje kao prihvatljiv, ali ovde imamo samo 17 difoltera na jednu varijablu ciji bin ne ulazi breko 
...
```
Dobili smo IV oko 0.04 sto zadovoljava medjutim jako je mali broj difolta u kategoriji 3 imajuci u vidu broj opstina koje spadaju u nju, spajamo 2 i 3
```{r}
...
```
```{r}
 model2<-  glm(formula =default.y~`Racio_novcane_likvidnosti_(Cash_ratio)`+Stepen_zaduzenosti+Gotovinski_ciklus_1+Vreme_naplate_potrazivanja+Stopa_prinosa_na_sopstveni_kapital_pre_oporezivanja+Cena_tudjih_izvora_sredstava, family = binomial(link = "logit"), 
    data = finalni_lrge)
...
names(my.vars)[c(1,2,4)]<-c("Rigorozni_racio_redukovane_monetarne_likvidnosti","Racio_novcane_likvidnosti","Racio_pokrica_kamata_zaradom_pre_kamata_i_poreza")
names(my.vars.test)[c(1,2,4)]<-c("Rigorozni_racio_redukovane_monetarne_likvidnosti","Racio_novcane_likvidnosti","Racio_pokrica_kamata_zaradom_pre_kamata_i_poreza")
...
#browser()
  ...
  auc.p[j] <- auc_roc(as.numeric(model.data.frame$fit),as.numeric(model.data.frame$dif))
  
  model.pred<-as.numeric(predict(model, newdata =my.vars.test, type = "response"))
...
result.frame <- data.frame(combination=sum.vars, auc.p=auc.p, auc.test=auc.pred,num_of_vars=num.of.vars)
...
Konacno biramo ovu formulu:  
 ...
 glm(formula =default.y~`Racio_novcane_likvidnosti_(Cash_ratio)`+Stepen_zaduzenosti+Gotovinski_ciklus_1+Vreme_naplate_potrazivanja+Stopa_prinosa_na_sopstveni_kapital_pre_oporezivanja+Cena_tudjih_izvora_sredstava, family = binomial(link = "logit"), 
    data = finalni_lrge)`  
______________________________________
