...
if(require("speedglm")==F){install.packages("speedglm");require("speedglm")}
if(require("mltools")==F){install.packages("mltools");require("mltools")}
...
data_opstine_po_razvijenosti$ Razvijenost<-factor(data_opstine_po_razvijenosti$ Razvijenost)
...
```
Dobili smo IV oko 0.09 sto zadovoljava 
...
data_2opstine$Sifra_opstine=factor(data_2opstine$Sifra_opstine)
...
```{r}
data_strani_investitor<-lrge.training.kategoricke[,c("default.y","Strani_investitor")]
data_strani_investitor$Strani_investitor<-factor(data_strani_investitor$Strani_investitor)
IV_strani_investitor<-create_infotables(data=data_strani_investitor,
                        y="default.y", 
                        parallel=FALSE)

IV_strani_investitor$Tables
```
Imamo IV oko 0.024. Uzecemo ovako.
...
odabrani<-odabrani[!odabrani=="Racio_obrta_poslovne_imovine"]

odabrani<-c(odabrani,"Asset_turnover")
odabrani
```



 Nismo proveravali ovde redove ali bi trebalo kada budemo radila sa malim i srednjim preduzecima. Sredimo sada `NA` vrednosti:
...
```{r}
model1 <- glm(formula = default.y ~ Stopa_prinosa_na_ukupna_sredstva_pre_oporezivanja + 
    `Racio_novcane_likvidnosti_(Cash_ratio)` + Stepen_zaduzenosti + 
    T12 + Razvijenost + Vreme_naplate_potrazivanja + T11 + Vreme_kreditiranja_kupaca, 
    family = binomial(link = "logit"), data = finalni_lrge)
...
```{r}
 model2<-glm(formula = default.y ~ `Racio_novcane_likvidnosti_(Cash_ratio)` + 
    ...
    Rast_EBITDA + Stopa_prinosa_na_ukupna_sredstva_pre_oporezivanja + 
    Cena_tudjih_izvora_sredstava + T11 + T12 + T14 + udeo_u_kapitalu + 
    Asset_turnover + Strani_investitor + Razvijenost, family = binomial(link = "logit"), 
    data = finalni_lrge)
summary(model2)
...
my.vars.test <- finalni_lrge.test# a matrix with your 14 different environmental variables

names(my.vars)[c(1,3)]<-c("Racio_novcane_likvidnosti","Racio_pokrica_kamata_zaradom_pre_kamata_i_poreza")
names(my.vars.test)[c(1,3)]<-c("Racio_novcane_likvidnosti","Racio_pokrica_kamata_zaradom_pre_kamata_i_poreza")
...
dif<-as.numeric(my.vars.test$default.y)
...
num.of.vars <- apply(as.data.frame(comb.mat),c(2)
                      , function(x) {
...
#browser()
  ...
  auc.p[j] <-
  auc_roc(as.numeric(model.data.frame$fit),
  as.numeric(model.data.frame$dif))
  HL.p[j] <-
  hoslem.test(model.data.frame$dif,
  as.numeric(model.data.frame$fit))$p.value
  
  model.pred <-
  as.numeric(predict(model, newdata = my.vars.test, type = "response"))
  auc.pred[j] <- auc_roc(model.pred, dif)
  HL.v[j] <- hoslem.test(dif, model.pred)$p.value
...
result.frame <- data.frame(combination=sum.vars, auc.p=auc.p, auc.test=auc.pred,num_of_vars=num.of.vars, HL.p=HL.p,HL.v=HL.v )

result.frame.sorted <- result.frame[order(-auc.p,-auc.pred,-num.of.vars), ]

tmp.result<-result.frame.sorted[result.frame.sorted$auc.test>0.74 & result.frame.sorted$auc.p>0.75 & result.frame.sorted$num_of_vars<9 & result.frame.sorted$HL.p>0.1 &result.frame.sorted$HL.v>0.1,]
```
...
Konacno se odlucujemo za sledeci model

```{r,results='asis'}
model.large.final <-
  glm(
  formula = default.y~`Racio_novcane_likvidnosti_(Cash_ratio)`+Stepen_zaduzenosti+Racio_obrta_poslovne_imovine+Gotovinski_ciklus_1+Vreme_naplate_potrazivanja+Stopa_prinosa_na_ukupna_sredstva_pre_oporezivanja,
  family = binomial(link = "logit"),
  data =finalni_lrge
  )
summary(model.large.final)
stargazer::stargazer(model.large.final,type = 'html')
...
model.large.data.frame=data.frame(fit=fitted.values(model.large.final), dif=model$y)
model.pred<-data.frame(fit=predict(model.large.final, newdata =finalni_lrge.test, type = "response"), dif=dif)

ROC_plot.large.train <-
  ggplot(model.large.data.frame, aes(m = fit, d = as.numeric(dif))) + geom_roc() +
  geom_abline(intercept = 0,
              slope = 1,
              color = 'red')

result.large.train = auc(model.large.data.frame$dif, model.large.data.frame$fit)

a = paste("Area under the curve:",
          round(ci(result.large.train)[[1]], 3),
          ", ",
          round(result.large.train, 3),
          ", ",
          round(ci(result.large.train)[[3]], 3))

ROC_plot.large.train <- ROC_plot.large.train + labs(title = a)+style_roc()

#validation
ROC_plot.large.validation <-
  ggplot(model.pred, aes(m = fit, d = as.numeric(dif))) + geom_roc() +
  geom_abline(intercept = 0,
              slope = 1,
              color = 'red')

result.validation <- auc(model.pred$dif, model.pred$fit)
...
ROC_plot.large.validation <- ROC_plot.large.validation + labs(title = a)+style_roc()
...
at=hoslem.test(model.large.data.frame$dif, model.large.data.frame$fit)
at
...
av=hoslem.test(as.numeric(model.pred$dif),
      as.numeric(model.pred$fit))
av
```
