...
```{r}
Pokazatelji_stari <- read_delim("C:/Users/jelena.cvetic/Desktop/Early warning/Razvojni folder/Bottom Up/Korak 3/Pokazatelji/Pokazatelji stari.csv",";", escape_double = FALSE, col_types = cols(Datum = col_date(format = "%m-%d-%Y")), locale = locale(encoding = "ASCII"), trim_ws = TRUE, progress =FALSE)
gc()
```

Ucitamo finansijska racija posle 2013. godine
```{r, eval=FALSE}
Pokazatelji_novi <- read_delim("C:/Users/jelena.cvetic/Desktop/Early warning/Razvojni folder/Bottom Up/Korak 3/Pokazatelji/Pokazatelji novi.csv", ";", escape_double = FALSE, col_types = cols(Datum = col_date(format = "%m-%d-%Y")),locale = locale(encoding = "ASCII"), trim_ws = TRUE, progress =FALSE)
gc()
```
...
KA4_url="C:/Users/jelena.cvetic/Desktop/Baze podataka/K4 i APR i NPL/KA4_podaci.txt"
RK_url="C:/Users/jelena.cvetic/Desktop/Baze podataka/K4 i APR i NPL/RK_novi.txt"
...
c=merge(NBS_data,APR_data,by.x = c("MAT_BR_DUZNIKA","DATUM.x"),by.y = c("Jmb","Datum"))
...
formatRound(
            datatable(
                      tr_summary_table1,caption = "Tabela 1.:Sumarni prikaz",
                      filter = 'none'
                      ),
...
Pravna_forma <- read_delim("C:/Users/milos.cipovic/Desktop/Projekti/Early warning/Razvojni folder/Bottom Up/Korak 5/Pravna forma.csv", "|", escape_double = FALSE, trim_ws = TRUE,col_types = cols(Datum = col_date(format = "%Y")), progress =FALSE)
...
clean_b<-merge(clean_b,Pravna_forma,by.x=c("DATUM.x","MAT_BR_DUZNIKA"),by.y = c("Datum1","Jmb"))
clean_b_<-clean_b

clean_b<-clean_b[Sifra==1400][,c("Naziv","Sifra","NazivPravnogLica"):=NULL]
...
#ucitavam seriju re4gulatornog kapitala za banke
 regulatory_capital <- read_delim(
   ...
   ";", escape_double = FALSE, col_types = cols(Datum = col_date(format = "%d.%m.%Y")),trim_ws = TRUE)

#prilepljujem je za clean_b tabelu
clean_b<-merge(clean_b,regulatory_capital,by.x = c("DATUM.x","MATICNI_BROJ.x"),by.y = c("Datum","MB"))

#proracunavam udeo izlozenosti i odstranjujem kolone viska koje su dosle sa tabelom regulatory_capital
clean_b<-clean_b[,udeo_u_kapitalu:=(IZLOZENOST.x/Brojilac)*1000]#skalirano zbog numerike
clean_b<-clean_b[,c("Brojilac","Imenilac","Pokazatelj"):=NULL]

#cistim smece
rm(b,Pravna_forma,regulatory_capital,summary_table,summary_table1,tr_summary_table)
gc()
```
...
![Tabela 4.: Polazni raciji koji ulaze u univariate analizu](tabela2.jpg){#id .class width=1500 height=1000}
...
Detaljno analiziramo **sredja preduzeca** (podaci `medium`) počevši od svih gore navedenih koraka u analizi, tako da će ovaj deo imati tri podgrupe, zavisno od same veličine dužnika. Samim tim i krećemo u detaljnu analizu sada...  

<h3 style="color:red;">Univariate analiza srednjih preduzeća</h3>
...
broj.red.medium<-nrow(medium)
broj.difolta.medium<-sum(as.numeric(medium$default.y==1))
procenat.difolta.medium<-broj.difolta.medium/broj.red.medium
```
...
```{r}
set.seed(42);#zbog reprodukcije
sample.medium<-sample(broj.red.medium,size=round(0.70*broj.red.medium,0))
medium.training<-medium[sample.medium,]
medium.test<-medium[-sample.medium,]
```
...
```{r}
#rpivotTable(medium.training)
summary_table.medium<-t(sapply(medium.training[,11:43],my.summary,arg=T))
tr_summary_table<-t(summary_table.medium)
formatRound(
            datatable(
                      summary_table.medium,caption = "Tabela 3.:Sumarni prikaz",
                      filter = 'none'
                      ),
            columns = colnames(summary_table.medium)
           )
```
...
medium.training.continualne<-medium.training[,c(7,6,11,13:43)]
medium.training.kategoricke<-medium.training[,c(7,8,9,10,12)]

medium.test.continualne<-medium.test[,c(7,6,11,13:43)]
medium.test.kategoricke<-medium.test[,c(7,8,9,10,12)]
...
ploting(medium.training,25,7)

```
*   Radna hipoteza-nezadovoljena, veci difolt kod manjih vrednosti
*   Diskriminativnost - ima na 95% znacajnosti ali je los oblik krive na pocetku i na kraju
*   autlajeri - naravno ima ih
*   normalnost - ma da...
...
ploting(medium.training,28,7)

```

*   Radna hipoteza-ne znam koji mu je djavo ali evo ovde cemo medijana zdravog je `r median(as.matrix(medium.training[default.y==0,28]),na.rm=T)` a nesolventnog je `r median(as.matrix(medium.training[default.y==1,28]),na.rm=T)` sto odgovara radnoj hipotezi
*   Diskriminativnost - znacajno na 95% znacajnosti
*   autlajeri - naravno ima ih
*   normalnost - ma da...
...
ploting(medium.training,31,7)

```

*   Radna hipoteza-zadovoljena je ali nema vidljive razlike
*   Diskriminativnost - znacajna na 95% znacajnosti ali krivuda možda kategoricka
*   autlajeri - naravno ima ih
*   normalnost - jook...
...
ploting(medium.training,34,7)

```
*   Radna hipoteza-zadovoljena je
*   Diskriminativnost - znacajna na 95% znacajnosti, medjutim brine sto je los oblik na pocetku krive
*   autlajeri - naravno ima ih
*   normalnost - jook...
...
ploting(medium.training,37,7)

```


*   Radna hipoteza-zadovoljena je
*   Diskriminativnost - ima na 95%
*   autlajeri - naravno ima ih
*   normalnost - jook...
...
ploting(medium.training,38,7)

```

*   Radna hipoteza-zadovoljena je
*   Diskriminativnost - ima ali krivuda
*   autlajeri - naravno ima ih
*   normalnost - jook...
...
U tabeli 5. ispod vidimo da najvecu diskriminacionu moc poseduje varijabla Stopa prinosa na ukupna sredstva pre oporeyivanja odmah iza koje je ALtman 1. 
```{r, message=F, warning=F}
#proracun
cor.medium=cor(medium.training.continualne[,-1], use = "complete.obs")

#dodajemo AUROC kao dodatnu kolonu pored varijable

corr_summary<-function (predictor){
  response=factor(medium.training.continualne[[1]])
suppressMessages(auc(response, as.numeric(predictor)))
}

auc_sumarno.medium<-sapply(medium.training.continualne[,-1], corr_summary)
kor_diskr.medium<-(cbind(auc_sumarno.medium,cor.medium)) 

formatRound(
            datatable(
                      kor_diskr.medium,caption = "Tabela 5.:Sumarni prikaz",
                      filter = 'none'
                      ),
            columns = colnames(kor_diskr.medium)
           )
...
clean_cor.medium<-corr_ellimination(kor_diskr.medium)
knitr::kable(clean_cor.medium, caption = "Tabela 6. skracena korelaciona tabela koja sadrzi varijable nad kojima ce se vrsiti dalja analiza")
...
data<-medium.training[,c("default.y","Pokrice_neto_kamata")]
...
#ovo je bitno
Pokrice_neto_kamata=calibrate_parameters(medium.training,"Pokrice_neto_kamata","default.y",broj_binova=10)

Pokrice_neto_kamata$auc.vrednost

Pokrice_neto_kamata$ostali_plotovi_posle
head(Pokrice_neto_kamata$Pokrice_neto_kamata.tr)
```
...
```{r}
Asset_turnover=calibrate_parameters(medium.training,"Asset_turnover","default.y")
...
```{r}
rpivotTable(medium.training.kategoricke,
            rows = "default.y", 
...
freq_table(medium.training.kategoricke,"Sifra_sektor","default.y")
```

Posmatrajuci tabelu grupisali smo sektore sa sli?nim stopama difolta (A, B, C, F, H, J), (I, D), (E, G, K, L, M, N, R, S).  
```{r}
medium.training.kategoricke$Sifra_sektor1<-medium.training.kategoricke$Sifra_sektor
medium.training.kategoricke$Sifra_sektor1[medium.training.kategoricke$Sifra_sektor%in%c("A","H","G")]<-"AHG"
medium.training.kategoricke$Sifra_sektor1[medium.training.kategoricke$Sifra_sektor%in%c("I","J")]<-"IJ"
medium.training.kategoricke$Sifra_sektor1[medium.training.kategoricke$Sifra_sektor%in%c("S","R","N","L","K","B")]<-"Ostalo"

medium.test.kategoricke$Sifra_sektor1<-medium.test.kategoricke$Sifra_sektor
medium.test.kategoricke$Sifra_sektor1[medium.test.kategoricke$Sifra_sektor%in%c("A","H","G")]<-"AHG"
medium.test.kategoricke$Sifra_sektor1[medium.test.kategoricke$Sifra_sektor%in%c("I","J")]<-"IJ"
medium.test.kategoricke$Sifra_sektor1[medium.test.kategoricke$Sifra_sektor%in%c("S","R","N","L","K","B")]<-"Ostalo"



freq_table(medium.training.kategoricke,"Sifra_sektor1","default.y")
```

```{r}
data_sifra_sektor.medium<-medium.training.kategoricke[,c("default.y","Sifra_sektor1")]

IV_sifra_sektor.medium<-create_infotables(data=data_sifra_sektor.medium,
                        y="default.y", 
                        parallel=FALSE)

IV_sifra_sektor.medium$Tables
```
*Information value* od 0.09 deluje kao prihvatljiv, iako ovde imamo samo 19 difoltera...ali ipak ovo je rejting a ne scoring. 
...
```{r}
freq_table(medium.training.kategoricke,"Sifra_opstine","default.y")
```
Uvidjamo da Beograd, Novi Sad i Ni? jedini imaju preko 100 duznika, tako da ima smisla deliti na Beograd, Novi Sad, Ni? i ostale opstine. Medjutim, vidimo da je default rate u Beogradu oko 10% sto je ,opet, blizu default ratea uzorka, samim tim, i Beograd otpada, ali bi eventualno mogli koristiti Novi Sad kao posebnu kategoriju. Pokusacemo da uvidimo da li nam predlozena varijabla o razvijenosti opstina donosi nesto novo (izvrsena je mala korekcija kod Zvecanj-a gde je dodata 4 kategorija ekonomske razvijenosti buduci da je opstina sa Kosova):
...
medium.training.kategoricke$id  <- 1:nrow(medium.training.kategoricke)
medium.test.kategoricke$id  <- 1:nrow(medium.test.kategoricke)

medium.training.kategoricke<-merge(medium.training.kategoricke,opstine_razvijenost,all.x = T)
medium.test.kategoricke<-merge(medium.test.kategoricke,opstine_razvijenost,all.x = T)

medium.training.kategoricke<-medium.training.kategoricke[order(medium.training.kategoricke$id), ][,id:=NULL]
medium.test.kategoricke<-medium.test.kategoricke[order(medium.test.kategoricke$id), ][,id:=NULL]



data_razvijenost_ostine.medium2<-medium.training.kategoricke[,c("default.y","Razvijenost")]
data_razvijenost_ostine.medium2$Razvijenost<-as.character(data_razvijenost_ostine.medium2$Razvijenost)

IV_razvijenost2<-create_infotables(data=data_razvijenost_ostine.medium2,
                        y="default.y", 
                        parallel=FALSE)

IV_razvijenost2$Tables   
freq_table(medium.training.kategoricke,"Razvijenost","default.y")
```

Koristicemo ovo!Koriscenje WOE i IV u slucaju velikih preduzeca, generalno, nije izvodljivo, usled malog broja difoltera po klasi faktorske varijable ([^7] predlaze bar 50 difoltera po kategoriji klasifikacije), ipak pogledacemo ova dva statistika kao putokaz i imati u vidu predlog [^8] gde navodi za IV statistik sledece:
...
```{r}
freq_table(medium.training.kategoricke,"Strani_investitor","default.y")
```
Postoji mala razlika, hajde da vidimo IV:
```{r}
WOETable(X=as.factor(medium.training.kategoricke$Strani_investitor),Y=(medium.training.kategoricke$default.y-1)*(-1))
```
Bzvze
...
```{r}
summary_table.medium<-as.data.frame(summary_table.medium)
preziveli.NA.medium<-rownames(summary_table.medium[summary_table.medium$NAspercent<10,])
```

Ciscenje usled korelacija i AUROC-a manjeg od 0.55
```{r}
preziveli.corr.AUC.medium<-row.names(clean_cor.medium)
```

Presek ova dva nastavlja u multivariate.

```{r}
odabrani.medium<-c(intersect(preziveli.NA.medium,preziveli.corr.AUC.medium))
```
...
finalni_medium<-medium.training[,c(odabrani.medium),with=F]
finalni_medium.test<-medium.test[,c(odabrani.medium),with=F]
...
finalni_medium<-as.data.table(sapply(finalni_medium,replace_outlier_with_quantile))
finalni_medium.test<-as.data.table(sapply(finalni_medium.test,replace_outlier_with_quantile))

finalni_medium$default.y<-medium.training$default.y
finalni_medium<-replace_missing_with_knn(finalni_medium)
finalni_medium$default.y<-NULL

finalni_medium.test$default.y<-medium.test$default.y
finalni_medium.test<-replace_missing_with_knn(finalni_medium.test,training = F)
finalni_medium.test$default.y<-NULL
...
```{r}
finalni_medium <-
  cbind.data.frame(finalni_medium, medium.training.kategoricke[, c("Sifra_sektor1","Razvijenost","Strani_investitor")])

finalni_medium.test <-
  cbind.data.frame(finalni_medium.test, medium.test.kategoricke[, c("Sifra_sektor1","Razvijenost","Strani_investitor")])


#pretvaram kategoricke u faktor da bi ih glm posmatrao kao kategoricke
finalni_medium$Sifra_sektor1 <- as.factor(finalni_medium$Sifra_sektor1)
finalni_medium$Razvijenost <-as.factor(finalni_medium$Razvijenost)
finalni_medium$Strani_investitor <-as.factor(finalni_medium$Strani_investitor)

finalni_medium.test$Sifra_sektor1 <- as.factor(finalni_medium.test$Sifra_sektor1)
finalni_medium.test$Razvijenost <-as.factor(finalni_medium.test$Razvijenost)
finalni_medium.test$Strani_investitor <-as.factor(finalni_medium.test$Strani_investitor)
```
...
```{r}
my.vars.medium <-finalni_medium# a matrix with your 14 different environmental variables
names(my.vars.medium)[c(1, 3,14)] <-
  c("Racio_novcane_likvidnosti",
  "Racio_pokrica_kamata_zaradom_pre_kamata_i_poreza","Altman_I")

my.vars.medium.valid <- finalni_medium.test#
names(my.vars.medium.valid)[c(1, 3,14)] <-
  c("Racio_novcane_likvidnosti",
  "Racio_pokrica_kamata_zaradom_pre_kamata_i_poreza","Altman_I")

my.vars.medium$default.y=NULL

library(speedglm)

nvar.medium<-ncol(my.vars.medium)

#colnames(my.vars) <- paste("var", 1:nvar, sep="") # add row names "var1" - "var14"
my.grad.data.medium <- 1:nvar.medium
sum.vars.medium <- vector()
auc.p.medium <- vector()
auc.pred.smal<-vector()
HL.p.medium <- vector()
HL.v.medium<-vector()

comb.mat.medium <- matrix(numeric(0), nrow=nvar.medium, ncol=0) # initialise the matrix containing all combinations
dif.medium.valid<-medium.test$default.y


for ( i in 1:nvar.medium ) { # generate and store all possible combination of sums of the 14 variables
  
t.mat.medium <- combn(my.grad.data.medium, m=i)
  
comb.mat.medium <- cbind(comb.mat.medium, rbind(t.mat.medium, matrix(NA, ncol=dim(t.mat.medium)[2] , nrow=nvar.medium-i)))
}

colnms.medium<-colnames(my.vars.medium)
my.vars.medium$default.y=medium.training$default.y

num.of.vars.medium <- apply(as.data.frame(comb.mat.medium),c(2)
                      , function(x) {
                      sum(as.numeric(!is.na(x)))
                      })

for ( j in 1:dim(comb.mat.medium)[2] ) { # calculate and store the R2 for all combinations
  
  #sum.vec <- rowSums(my.vars[, comb.mat[, j]], na.rm=TRUE)
#if(j==13)browser()
  sum.vars.medium[j] <- paste( colnms.medium[c(na.omit(comb.mat.medium[, j]))], 
    collapse="+")
  relacija.medium=as.formula(paste("default.y ~ ",sum.vars.medium[j],sep = ""))
  model.medium = speedglm(relacija.medium,
                   data = my.vars.medium,
                   y=TRUE,
                   fitted = TRUE,
                   family = binomial(link = "logit"))

  model.medium.data.frame=data.frame(fit=fitted.values(model.medium), dif=model.medium$y)
  #browser()
  auc.p.medium[j] <- auc_roc(as.numeric(model.medium.data.frame$fit),
                            as.numeric(model.medium.data.frame$dif))
  HL.p.medium[j] <-hoslem.test(as.numeric(model.medium.data.frame$dif),
                        as.numeric(model.medium.data.frame$fit))$p.value
  #auc.p.medium[j] <-fastAUC(as.numeric(model.medium.data.frame$fit),as.numeric(model.medium.data.frame$dif))
  
  
  model.medium.pred<-as.numeric(predict(model.medium, newdata = my.vars.medium.valid, type = "response"))
  HL.v.medium[j] <- hoslem.test(dif.medium.valid, model.medium.pred)$p.value
  auc.pred.smal[j]<-auc_roc( model.medium.pred,dif.medium.valid)
  #auc.pred.smal[j]<-fastAUC( model.medium.pred,dif.medium.valid)
  
  if(j %in% round(seq(
    from = 1,
    to = dim(comb.mat.medium)[2],
    length.out = 100
    )))
    print(j / dim(comb.mat.medium)[2]
    )
...
my.vars.medium.valid$default.y=dif.medium.valid
my.vars.medium$default.y=medium.training$default.y


result.frame.medium <- data.frame(combination=sum.vars.medium, auc.p.medium=auc.p.medium, auc.valid.medium=auc.pred.smal,num_of_vars=num.of.vars.medium,HL.p.medium=HL.p.medium,HL.v.medium=HL.v.medium)

write.csv2(file = "srednji.csv",x=result.frame.medium)
head(result.frame.medium.sorted, n=100) # the 10 "best" combinations
```

krajnji model

```{r}
model.medium.final<-glm(formula=default.y~Racio_novcane_likvidnosti+Stepen_zaduzenosti+Racio_pokrica_kamata_zaradom_pre_kamata_i_poreza+Asset_turnover+Stopa_prinosa_na_ukupna_sredstva_pre_oporezivanja+Cena_tudjih_izvora_sredstava+Altman_I+udeo_u_kapitalu+Sifra_sektor1+Razvijenost+Strani_investitor, family = binomial(link = "logit"),
  data =my.vars.medium)
...
final.roc.plot(model.medium.final,my.vars.medium,my.vars.medium.valid)

```

```{r}
pocetna.sumarna.tabela_medium<-summary_table.medium
korerlacije.i.AUROC.pocetni_medium<-kor_diskr.medium
korerlacije.i.AUROC.odabrani_medium<-clean_cor.medium


precisceni.raciji.razvojni.uzorak_medium<-my.vars.medium
precisceni.raciji.testni.uzorak_medium<-my.vars.medium.valid

set.isprobanih.modela_medium<-result.frame.medium
izabrani.model_medium<-model.medium.final
zavrsni.plot.i.HL_medium<-final.roc.plot(model.medium.final,
                                        my.vars.medium,
                                        my.vars.medium.valid)

save(pocetna.sumarna.tabela_medium,
     korerlacije.i.AUROC.pocetni_medium,
     korerlacije.i.AUROC.odabrani_medium,
     precisceni.raciji.razvojni.uzorak_medium,
     precisceni.raciji.testni.uzorak_medium,
     set.isprobanih.modela_medium,
     izabrani.model_medium,
     zavrsni.plot.i.HL_medium,
     file = "mediumData.RData"
  )

#brisemo sve sto ima medium u nayivu, jbm ti i y i z i srpsku tastaturu i sve u picku materinu
remove(list = ls()[grepl("medium",ls())])
```
