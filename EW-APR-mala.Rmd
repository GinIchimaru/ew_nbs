...
small.test.continualne<-small.training[,c(7,6,11,13:43)]
small.test.kategoricke<-small.training[,c(7,8,9,10,12)]
...
Zakljucujemo da, sektor kod malih i mikro preduzeca ne igra bitnu ulogu.
####Sifra opstine:  
...
```{r}
odabrani.small<-c("Racio_novcane_likvidnosti_(Cash_ratio)", c(intersect(preziveli.NA.small,preziveli.corr.AUC.small)))
```
...
default.y<-small.training$default.y
finalni_small<-as.data.table(sapply(finalni_small,replace_missing_with_median))
```
...
Posto ni jedna kategoricka nije prosla filter necemo ih spajati
...
model1.small <- glm(formula = default.y ~ T12 + Stopa_prinosa_na_ukupna_sredstva_pre_oporezivanja + 
    Asset_turnover + udeo_u_kapitalu + Broj_zaposlenih + Vreme_kreditiranja_kupaca + 
    Stepen_zaduzenosti + `Racio_novcane_likvidnosti_(Cash_ratio)` + 
    T14, family = binomial(link = "logit"), data = finalni_small)
...
```{r}
 model2.small= glm(formula = default.y ~ `Racio_novcane_likvidnosti_(Cash_ratio)` + 
    Broj_zaposlenih + Stepen_zaduzenosti + `Racio_pokrica_kamata_zaradom_pre_kamata_i_poreza_(Interest_Coverage_Ratio)` + 
     + Gotovinski_ciklus_1 + Vreme_kreditiranja_kupaca + 
    ...
    T14 + udeo_u_kapitalu, family = binomial(link = "logit"), 
    data = finalni_small)
...
```{r}
finalni_small.valid<-small.test[,c("Racio_novcane_likvidnosti_(Cash_ratio)",odabrani),with=F]
finalni_small.valid<-as.data.table(sapply(finalni_small.valid, replace_outlier_with_quantile))


finalni_small.valid<-as.data.table(sapply(finalni_small.valid, replace_missing_with_median,training=F))


model1.small.pred<-as.numeric(predict(model1.small, newdata = finalni_small.valid, type = "response"))
...
```{r}
my.vars.small <-
  finalni_small# a matrix with your 14 different environmental variables
names(my.vars.small)[c(1, 4)] <-
  c("Racio_novcane_likvidnosti",
  "Racio_pokrica_kamata_zaradom_pre_kamata_i_poreza")

my.vars.small.valid <- finalni_small.valid#
